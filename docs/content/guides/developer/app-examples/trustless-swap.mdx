...
3. [Publish the contract and demo data](#demo-data)
...
Next, configure the Sui CLI to use `testnet` as the active environment. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
This aligns with the `tsconfig.json` setup, ensuring that both TypeScript and Vite understand how to resolve `@/` imports.

### Adding Tailwind CSS

This app uses Tailwind CSS for styling. Run the following command to add Tailwind CSS and its dependencies:
...
Create a `src/utils/` directory and add `helpers.ts`: 
...
```bash
pnpm add react-router-dom react-hot-toast
```
...
Use the `useSuiClient()` hook from `dApp Kit` to retrieve the Sui client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the Sui client instance of the Sui TypeScript SDK. Use `react-hot-toast` as another dependency to toast transaction status to users.
...
#### Finished demo mutations

At this point, view the UI and click `New Demo Bear` to create a dummy bear object. Note, you won't be able to view the bear until you implement future steps.
...
{@inject: examples/trading/frontend/src/mutations/locked.ts#mutationlock}
...
{@inject: examples/trading/frontend/src/mutations/locked.ts#mutationunlock}

#### Finished locked mutations

When finished, the `locked.ts` file should look like this:
...
{@inject: examples/trading/frontend/src/mutations/escrow.ts#mutationcreateescrow}
...
{@inject: examples/trading/frontend/src/mutations/escrow.ts#mutationacceptescrow}
...
{@inject: examples/trading/frontend/src/mutations/escrow.ts#mutationcancelescrow}

#### Finished escrow mutations

When finished, the `escrow.ts` file should look like this:

{@inject: examples/trading/frontend/src/mutations/escrow.ts}
...
The `Locked` component is a bit complex because it has multiple actions that can be performed on the locked object. The `Locked` component is responsible for rendering the `ApiLockedObject` and providing the necessary actions to interact with the locked object. The `Locked` component consists of the following actions:

- **Unlock** button to unlock the locked object. 
- **Create Escrow** button to create an escrow out of the locked object. This contains a complex UI so it is implemented in `src/components/escrows/CreateEscrow.tsx`.
...
#### Finished locked dashboard

At this point, view the `Managed Objects` page in the UI to see the list of owned objects and the ability to lock/unlock them.
...
The functionality to collect the locked object in an escrow object is used in multiple places. Hence, it's better to extract the logic into a separate hook to enhance reusability and encapsulation.
...
#### Finished escrow dashboard

At this point, view the `Escrows` page in the UI to see the list of escrows requested to you, browse locked objects to trade for, and browse escrows you have initiated for third-party locked objects.
